		...
		for (int j = 0; j != num; j++)
		{
			...
			if (mMDBone._isLateUpdatePosition || mMDBone._isLateUpdateRotation)
			{
				...
				if (mMDBone._isLateUpdatePosition)
				{
					...
					ref Vector3 reference = ref _updateData.lateUpdateBonePositionList[j];
					reference = mMDBone._unityLocalPosition;
				}
				if (mMDBone._isLateUpdateRotation)
				{
					_updateData.lateUpdateBoneFlagsList[j] |= 4;
					ref Quaternion reference2 = ref _updateData.lateUpdateBoneRotationList[j];
					reference2 = mMDBone._unityLocalRotation;
				}
			}
		}
		if (_isVertexMorphEnabled && !_isBlendShapesEnabled)
