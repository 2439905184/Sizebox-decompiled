...
public class MMDMorph
{
	...
	public bool isMorphBaseVertex => (_additionalFlags & 1) != 0;

	public MMDMorphCategory morphCategory => _morphCategory;
	...
	public bool Import(BinaryReader binaryReader)
	{
		...
		case MMDMorphType.Vertex:
		{
			...
			for (uint num2 = 0u; num2 != _indexCount; num2++)
			{
				...
				ref Vector3 reference3 = ref _vertices[num2];
				reference3 = binaryReader.ReadVector3();
		...
		case MMDMorphType.Bone:
		{
			...
			for (uint num = 0u; num != _indexCount; num++)
			{
				...
				ref IndexedVector3 reference = ref _bonePositions[num];
				reference = binaryReader.ReadVector3();
				ref IndexedQuaternion reference2 = ref _boneRotations[num];
				reference2 = binaryReader.ReadQuaternion();
