...
public class MMDIK
{
	...
	private struct IKLink
	{
		...
		public bool hasLimit => (ikLinkFlags & 1) != 0;
	}
	...
	private const float IKLimitInnerAngle = 3.07177949f;
	...
	public void PreUpdate_MarkIKDepended(uint updateFlags, float ikWeight)
	{
		...
		{
			_destBone.PreUpdate_MarkIKDepended(isIKLink: false, isIKTarget: false, isIKDestination: true);
			_targetBone.PreUpdate_MarkIKDepended(isIKLink: false, isIKTarget: true, isIKDestination: false);
			for (int i = 0; i < _ikLinkList.Length; i++)
			{
				_ikLinkList[i].bone.PreUpdate_MarkIKDepended(isIKLink: true, isIKTarget: false, isIKDestination: false);
			}
		}
	}
	...
	public void Solve()
	{
		...
		_PrepareIKTransform();
		_SolveInternal(secondPass: false);
		if (flag2)
		{
			if (_isSavedResults)
			{
				...
				if (flag3)
				{
					...
					_SolveInternal(secondPass: true);
				}
			}
	...
	private void _PrepareIKTransform()
	{
		...
		{
			_ikLinkList[num].bone.PrepareIKTransform(underIKLink: true, _ikWeight);
		}
		_targetBone.PrepareIKTransform(underIKLink: false, _ikWeight);
	}
