...
public class DynamicBone : MonoBehaviour
{
	...
	public HashSet<Transform> m_Exclusions = new HashSet<Transform>();
	...
	private void AppendParticles(Transform bone, int parentIndex, float boneLength)
	{
		Particle particle = new Particle();
		particle.m_Transform = bone;
		...
		foreach (Transform item in bone)
		{
			if (SkipBone(item))
			{
				SkipParticles(item, count, boneLength);
			}
			else if (!m_Exclusions.Contains(item))
			{
				AppendParticles(item, count, boneLength);
			}
		...
		if (bone.childCount == 0 && (m_EndLength > 0f || m_EndOffset != Vector3.zero))
		{
			AppendParticles(null, count, boneLength);
		}
	}

	private bool SkipBone(Transform transform)
	{
		if (!transform.GetComponent<NoJiggle>())
		{
			return transform.GetComponent<MeshCollider>();
		}
		return true;
	}

	private void SkipParticles(Transform bone, int parentIndex, float boneLength)
	{
		foreach (Transform item in bone)
		{
			if (SkipBone(item))
			{
				SkipParticles(item, parentIndex, boneLength);
			}
			else if (!m_Exclusions.Contains(item))
			{
				AppendParticles(item, parentIndex, boneLength);
			}
			else if (m_EndLength > 0f || m_EndOffset != Vector3.zero)
			{
				AppendParticles(null, parentIndex, boneLength);
			}
		}
		if (bone.childCount == 0 && (m_EndLength > 0f || m_EndOffset != Vector3.zero))
		{
