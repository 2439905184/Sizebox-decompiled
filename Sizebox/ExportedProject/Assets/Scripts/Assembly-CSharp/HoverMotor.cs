...
public class HoverMotor : Motor
{
	...
	{
		base.FixedUpdate();
		float f = (vp.brakeIsReverse ? (vp.accelInput - vp.brakeInput) : vp.accelInput);
		...
		foreach (HoverWheel hoverWheel in array)
		{
			if (ignition)
			{
				...
				hoverWheel.targetSpeed = actualInput * forceCurve.keys[forceCurve.keys.Length - 1].time * (boosting ? (1f + num) : 1f);
				hoverWheel.targetForce = Mathf.Abs(actualInput) * forceCurve.Evaluate(Mathf.Abs(vp.localVelocity.z) - (boosting ? num : 0f)) * power * (boosting ? (1f + num) : 1f) * health;
			}
			else
			{
