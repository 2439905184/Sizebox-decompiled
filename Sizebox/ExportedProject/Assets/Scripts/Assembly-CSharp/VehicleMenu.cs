...
public class VehicleMenu : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			stuntEndTime = (string.IsNullOrEmpty(stunter.stuntString) ? Mathf.Max(0f, stuntEndTime - Time.deltaTime) : 2f);
			if (stuntEndTime == 0f)
			{
				stuntText.text = "";
			}
	...
	public void SpawnVehicle(int vehicle)
	{
		...
		{
			newVehicle.GetComponent<FlipControl>().flipPower = ((stuntMode && assist) ? new Vector3(10f, 10f, -10f) : Vector3.zero);
			newVehicle.GetComponent<FlipControl>().rotationCorrection = (stuntMode ? Vector3.zero : (assist ? new Vector3(5f, 1f, 10f) : Vector3.zero));
