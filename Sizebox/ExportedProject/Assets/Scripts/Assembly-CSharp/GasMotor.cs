...
public class GasMotor : Motor
{
	...
	{
		base.FixedUpdate();
		actualAccel = Mathf.Lerp((vp.brakeIsReverse && vp.reversing && vp.accelInput <= 0f) ? vp.brakeInput : vp.accelInput, Mathf.Max(vp.accelInput, vp.burnout), vp.burnout);
		float f = (canReverse ? actualAccel : Mathf.Clamp01(actualAccel));
		...
		if (ignition)
		{
			...
			targetDrive.rpm = Mathf.Lerp(targetDrive.rpm, actualInput * maxRPM * 1000f * (boosting ? (1f + num) : 1f), (1f - inertia) * Time.timeScale);
			...
			{
				targetDrive.torque = torqueCurve.Evaluate(targetDrive.feedbackRPM * 0.001f - (boosting ? num : 0f)) * Mathf.Lerp(targetDrive.torque, power * (float)Mathf.Abs(Math.Sign(actualInput)), (1f - inertia) * Time.timeScale) * (boosting ? (1f + num) : 1f) * health;
			}
	...
	public override void Update()
	{
		...
		{
			airPitch = ((vp.groundedWheels > 0 || actualAccel != 0f) ? 1f : Mathf.Lerp(airPitch, 0f, 0.5f * Time.deltaTime));
			pitchFactor = ((actualAccel != 0f || vp.groundedWheels == 0) ? 1f : 0.5f) * ((!shifting) ? 1f : (pitchIncreaseBetweenShift ? Mathf.Sin(transmission.shiftTime / transmission.shiftDelay * (float)Math.PI) : (Mathf.Min(transmission.shiftDelay, Mathf.Pow(transmission.shiftTime, 2f)) / transmission.shiftDelay))) * airPitch;
