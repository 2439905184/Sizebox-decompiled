...
public class VehicleParent : MonoBehaviour
{
	...
	{
		brakeInput = (accelAxisIsBrake ? (0f - Mathf.Clamp(accelInput, -1f, 0f)) : Mathf.Clamp(f, -1f, 1f));
	}
	...
	private void SetCenterOfMass()
	{
		...
		if (suspensionCenterOfMass)
		{
			if (hover)
			{
				...
				{
					num = ((i == 0) ? hoverWheels[i].hoverDistance : ((num + hoverWheels[i].hoverDistance) * 0.5f));
				}
			}
			else
			{
				for (int j = 0; j < wheels.Length; j++)
				{
					...
					num = ((j == 0) ? suspensionDistance : ((num + suspensionDistance) * 0.5f));
				}
			}
		}
