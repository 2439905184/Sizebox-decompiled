...
public class TireMarkCreate : MonoBehaviour
{
	...
	private void Update()
	{
		if (w.grounded)
		{
			alwaysScrape = (GroundSurfaceMaster.surfaceTypesStatic[w.contactPoint.surfaceType].alwaysScrape ? (slipThreshold + Mathf.Min(0.5f, Mathf.Abs(w.rawRPM * 0.001f))) : 0f);
		}
		else
		{
			alwaysScrape = 0f;
		...
		if (w.grounded && (Mathf.Abs(F.MaxAbs(w.sidewaysSlip, w.forwardSlip)) > slipThreshold || alwaysScrape > 0f) && w.connected)
		{
			...
			if ((bool)curMark)
			{
				Vector3 vector = Quaternion.AngleAxis(90f, w.contactPoint.normal) * tr.right * (w.popped ? w.rimWidth : w.tireWidth);
	...
	private void UpdateMark()
	{
		if (gapDelay == 0f)
		{
			...
			{
				verts[i] = (Mathf.Approximately((float)i * 0.5f, Mathf.Round((float)i * 0.5f)) ? leftPoint : rightPoint);
				colors[i].a = 0f;
			}
		...
		else
		{
			...
			{
				verts[j] = (Mathf.Approximately((float)j * 0.5f, Mathf.Round((float)j * 0.5f)) ? leftPoint : rightPoint);
				colors[j].a = 0f;
			}
			mesh.vertices = verts;
			mesh.RecalculateBounds();
