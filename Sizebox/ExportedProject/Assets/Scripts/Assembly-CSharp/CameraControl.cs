...
public class CameraControl : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		if ((bool)target && (bool)targetBody && target.gameObject.activeSelf)
		{
			...
			{
				targetForward = (stayFlat ? new Vector3(vp.norm.up.x, 0f, vp.norm.up.z) : vp.norm.up);
			}
			targetUp = (stayFlat ? GlobalControl.worldUpDir : vp.norm.forward);
			lookDir = Vector3.Slerp(lookDir, (xInput == 0f && yInput == 0f) ? Vector3.forward : new Vector3(xInput, 0f, yInput).normalized, 0.1f * TimeMaster.inverseFixedTimeFactor);
			smoothYRot = Mathf.Lerp(smoothYRot, targetBody.angularVelocity.y, 0.02f * TimeMaster.inverseFixedTimeFactor);
			RaycastHit hitInfo;
			if (Physics.Raycast(target.position, -targetUp, out hitInfo, 1f, castMask) && !stayFlat)
			{
				upLook = Vector3.Lerp(upLook, ((double)Vector3.Dot(hitInfo.normal, targetUp) > 0.5) ? hitInfo.normal : targetUp, 0.05f * TimeMaster.inverseFixedTimeFactor);
			}
			else
			{
