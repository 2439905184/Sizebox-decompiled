...
public class TerrainToolkit : MonoBehaviour
{
	...
	private void erodeTerrainWithBrush()
	{
		...
		try
		{
			...
			int num3 = (int)Mathf.Floor((float)heightmapResolution / size.x * brushSize);
			int num4 = (int)Mathf.Floor((float)heightmapResolution2 / size.z * brushSize);
			Vector3 vector = base.transform.InverseTransformPoint(brushPosition);
			num = (int)Mathf.Round(vector.x / size.x * (float)heightmapResolution - (float)(num3 / 2));
			num2 = (int)Mathf.Round(vector.z / size.z * (float)heightmapResolution2 - (float)(num4 / 2));
			if (num < 0)
			{
				num3 += num;
				num = 0;
			...
			if (num + num3 > heightmapResolution)
			{
				num3 = heightmapResolution - num;
			}
			if (num2 + num4 > heightmapResolution2)
			{
				num4 = heightmapResolution2 - num2;
			}
	...
	public void erodeAllTerrain(ErosionProgressDelegate erosionProgressDelegate)
	{
		...
		try
		{
			...
			int heightmapResolution = terrainData.heightmapResolution;
			int heightmapResolution2 = terrainData.heightmapResolution;
			float[,] array = terrainData.GetHeights(0, 0, heightmapResolution, heightmapResolution2);
			switch (erosionType)
			{
			default:
				return;
			case ErosionType.Thermal:
			{
				...
				array = fastErosion(array, new Vector2(heightmapResolution, heightmapResolution2), iterations, erosionProgressDelegate);
				break;
			}
			case ErosionType.Hydraulic:
			{
				...
				case HydraulicType.Full:
					array = fullHydraulicErosion(array, new Vector2(heightmapResolution, heightmapResolution2), iterations, erosionProgressDelegate);
					break;
				case HydraulicType.Velocity:
					array = velocityHydraulicErosion(array, new Vector2(heightmapResolution, heightmapResolution2), iterations, erosionProgressDelegate);
					break;
				}
				break;
			}
			...
			case ErosionType.Wind:
			{
				...
				array = windErosion(array, new Vector2(heightmapResolution, heightmapResolution2), iterations, erosionProgressDelegate);
				break;
			}
			}
		...
		for (int i = 0; i < iterations; i++)
		{
			for (int j = 0; j < num2; j++)
			{
				...
				for (int k = 0; k < num; k++)
				{
					...
					{
						float num37 = array[k + num19 + num18, j + num16 + num15] - num35;
						array[k + num19 + num18, j + num16 + num15] = num37;
					}
					else
					{
						...
						heightMap[k + num19 + num18, j + num16 + num15] = num38;
					}
					for (int l = 0; l < num14; l++)
					{
						for (int m = 0; m < num17; m++)
						{
							...
							float num39 = heightMap[k + m + num18, j + l + num15];
							float num27 = num36 - num39;
							if (!(num27 > 0f))
							{
								continue;
							}
							float num40 = num35 * (num27 / num22);
							if (erosionMode == ErosionMode.Filter || (erosionMode == ErosionMode.Brush && useDifferenceMaps))
							{
								float num41 = array[k + m + num18, j + l + num15] + num40;
								array[k + m + num18, j + l + num15] = num41;
								continue;
							}
							num39 += num40;
							if (num39 < 0f)
							...
							heightMap[k + m + num18, j + l + num15] = num39;
						}
					}
				}
			}
			if ((erosionMode == ErosionMode.Filter || (erosionMode == ErosionMode.Brush && useDifferenceMaps)) && erosionType != ErosionType.Tidal)
			{
				for (int j = 0; j < num2; j++)
				{
					for (int k = 0; k < num; k++)
					{
						float num42 = heightMap[k, j] + array[k, j];
						if (num42 > 1f)
						{
							num42 = 1f;
						}
			...
			if (erosionMode == ErosionMode.Filter)
			{
				...
				erosionProgressDelegate(text, text2, i, iterations, percentComplete);
			}
		}
		return heightMap;
	}
		...
		for (int m = 0; m < iterations; m++)
		{
			...
			for (int i = 0; i < num2; i++)
			{
				...
				for (int j = 0; j < num; j++)
				{
					...
					float num30 = array4[j, i] - num29;
					array4[j, i] = num30;
					float num31 = num26 * (num29 / num21);
					float num32 = num25 * (num29 / num21);
					for (int k = 0; k < num4; k++)
					{
						for (int l = 0; l < num7; l++)
						{
							if ((l != num9 || k != num6) && (neighbourhood == Neighbourhood.Moore || (neighbourhood == Neighbourhood.VonNeumann && (l == num9 || k == num6))))
							{
								...
								if (num14 > 0f)
								{
									float num33 = array4[j + l + num8, i + k + num5];
									float num34 = num29 * (num14 / num10);
									float num35 = num33 + num34;
									array4[j + l + num8, i + k + num5] = num35;
									float num36 = array6[j + l + num8, i + k + num5];
									float num37 = num31 * hydraulicMomentum * (num14 / num10);
									float num38 = num36 + num37;
									array6[j + l + num8, i + k + num5] = num38;
									float num39 = array8[j + l + num8, i + k + num5];
									float num40 = num32 * hydraulicMomentum * (num14 / num10);
									float num41 = num39 + num40;
									array8[j + l + num8, i + k + num5] = num41;
								}
							}
						}
					}
					float num42 = array6[j, i];
					array6[j, i] = num42 - num31;
				}
			}
			for (int i = 0; i < num2; i++)
			{
				for (int j = 0; j < num; j++)
				{
					float num43 = array5[j, i] + array6[j, i];
					num43 *= 1f - hydraulicEntropy;
					if (num43 > 1f)
					{
						num43 = 1f;
					}
			...
			for (int i = 0; i < num2; i++)
			{
				for (int j = 0; j < num; j++)
				{
					float num44 = array3[j, i] + array4[j, i];
					float num45 = num44 * hydraulicVelocityEvaporation;
					num44 -= num45;
					if (num44 > 1f)
					{
						num44 = 1f;
			...
			for (int i = 0; i < num2; i++)
			{
				for (int j = 0; j < num; j++)
				{
					float num46 = array7[j, i] + array8[j, i];
					if (num46 > 1f)
					{
						num46 = 1f;
					}
			...
			for (int i = 0; i < num2; i++)
			{
				...
				{
					float num18 = array3[j, i] * hydraulicVelocitySedimentSaturation;
					float num46 = array7[j, i];
					if (num46 > num18)
					{
						float num47 = num46 - num18;
						array7[j, i] = num18;
						float num48 = heightMap[j, i];
						heightMap[j, i] = num48 + num47;
					}
				}
			}
			for (int i = 0; i < num2; i++)
			{
				for (int j = 0; j < num; j++)
				{
					float num43 = array3[j, i];
					float num48 = heightMap[j, i];
					float num49 = 1f - Mathf.Abs(0.5f - num48) * 2f;
					float num50 = hydraulicDowncutting * num43 * num49;
					num48 -= num50;
					heightMap[j, i] = num48;
				}
			}
		...
		for (int k = 0; k < iterations; k++)
		{
			...
			for (int i = 0; i < num2; i++)
			{
				...
				for (int j = 0; j < num; j++)
				{
					...
					float num24 = array2[j + num13 + num12, i + num10 + num9] - num23;
					array2[j + num13 + num12, i + num10 + num9] = num24;
					float num25 = array3[j + num13 + num12, i + num10 + num9] * (num23 / num16);
					float num26 = array4[j + num13 + num12, i + num10 + num9] - num25;
					array4[j + num13 + num12, i + num10 + num9] = num26;
					for (int l = 0; l < num8; l++)
					{
						for (int m = 0; m < num11; m++)
						{
							if ((m != num13 || l != num10) && (neighbourhood == Neighbourhood.Moore || (neighbourhood == Neighbourhood.VonNeumann && (m == num13 || l == num10))))
							{
								...
								if (num21 > 0f)
								{
									float num27 = array2[j + m + num12, i + l + num9];
									float num28 = num23 * (num21 / num14);
									float num29 = num27 + num28;
									array2[j + m + num12, i + l + num9] = num29;
									float num30 = array4[j + m + num12, i + l + num9];
									float num31 = num25 * (num21 / num14);
									float num32 = num30 + num31;
									array4[j + m + num12, i + l + num9] = num32;
								}
							}
						}
					}
			...
			for (int i = 0; i < num2; i++)
			{
				for (int j = 0; j < num; j++)
				{
					float num33 = array[j, i] + array2[j, i];
					float num34 = num33 * hydraulicEvaporation;
					num33 -= num34;
					if (num33 < 0f)
					{
						num33 = 0f;
			...
			for (int i = 0; i < num2; i++)
			{
				for (int j = 0; j < num; j++)
				{
					float num35 = array3[j, i] + array4[j, i];
					if (num35 > 1f)
					{
						num35 = 1f;
					}
			...
			for (int i = 0; i < num2; i++)
			{
				...
				{
					float num5 = array[j, i] * hydraulicSedimentSaturation;
					float num35 = array3[j, i];
					if (num35 > num5)
					{
						float num36 = num35 - num5;
						array3[j, i] = num5;
						float num37 = heightMap[j, i];
						heightMap[j, i] = num37 + num36;
					}
				}
			}
		...
		for (int k = 0; k < iterations; k++)
		{
			...
			for (int i = 0; i < num2; i++)
			{
				for (int j = 0; j < num; j++)
				{
					...
					float num11 = (array3[j, i] += num10);
					float num12 = array5[j, i];
					float num13 = windLift * num11;
					if (num12 + num13 > windCapacity)
					{
						num13 = windCapacity - num12;
					}
					array5[j, i] = num12 + num13;
					heightMap[j, i] = num7 - num13;
				}
			}
			for (int i = 0; i < num2; i++)
			{
				...
				for (int j = 0; j < num; j++)
				{
					...
					float num20 = array2[j, i];
					float num21 = array[j, i];
					float num12 = array5[j, i];
					for (int l = 0; l < num14; l++)
					{
						for (int m = 0; m < num17; m++)
						{
							...
							{
								Vector3 from = new Vector3(m + num18, 0f, -1 * (l + num15));
								float num22 = (90f - Vector3.Angle(from, to)) / 90f;
								if (num22 < 0f)
								{
									num22 = 0f;
								}
								float num23 = array4[j + m + num18, i + l + num15];
								float num24 = num22 * (num20 - num21) * 0.1f;
								if (num24 < 0f)
								{
									num24 = 0f;
								}
								float num25 = num23 + num24;
								array4[j + m + num18, i + l + num15] = num25;
								float num26 = array4[j, i] - num24;
								array4[j, i] = num26;
								float num27 = array6[j + m + num18, i + l + num15];
								float num28 = num12 * num24;
								float num29 = num27 + num28;
								array6[j + m + num18, i + l + num15] = num29;
								float num30 = array6[j, i] - num28;
								array6[j, i] = num30;
							}
						}
					}
			...
			for (int i = 0; i < num2; i++)
			{
				for (int j = 0; j < num; j++)
				{
					float num31 = array5[j, i] + array6[j, i];
					if (num31 > 1f)
					{
						num31 = 1f;
					}
			...
			for (int i = 0; i < num2; i++)
			{
				for (int j = 0; j < num; j++)
				{
					float num32 = heightMap[j, i];
					float num33 = heightMap2[j, i];
					float num34 = array[j, i] * windSmoothing;
					float num35 = num33 * num34 + num32 * (1f - num34);
					heightMap[j, i] = num35;
				}
			}
			float percentComplete = (float)k / (float)iterations;
	...
	public void textureTerrain(TextureProgressDelegate textureProgressDelegate)
	{
		...
		int num2 = terrainData.heightmapResolution - 1;
		int num3 = terrainData.heightmapResolution - 1;
	...
	public void generateTerrain(GeneratorProgressDelegate generatorProgressDelegate)
	{
		...
		case GeneratorType.Voronoi:
			heightMap = generateVoronoi(heightMap, new Vector2(heightmapResolution, heightmapResolution2), generatorProgressDelegate);
			break;
		case GeneratorType.DiamondSquare:
			heightMap = generateDiamondSquare(heightMap, new Vector2(heightmapResolution, heightmapResolution2), generatorProgressDelegate);
			break;
		case GeneratorType.Perlin:
			heightMap = generatePerlin(heightMap, new Vector2(heightmapResolution, heightmapResolution2), generatorProgressDelegate);
			break;
		case GeneratorType.Smooth:
			heightMap = smooth(heightMap, new Vector2(heightmapResolution, heightmapResolution2), generatorProgressDelegate);
			break;
		case GeneratorType.Normalise:
			heightMap = normalise(heightMap, new Vector2(heightmapResolution, heightmapResolution2), generatorProgressDelegate);
			break;
		}
	...
	private float[,] generateVoronoi(float[,] heightMap, Vector2 arraySize, GeneratorProgressDelegate generatorProgressDelegate)
	{
		...
		for (int j = 0; j < num2; j++)
		{
			for (int k = 0; k < num; k++)
			{
				...
				for (int i = 0; i < voronoiCells; i++)
				{
					float dist = Vector2.Distance(((Peak)arrayList[i]).peakPoint, new Vector2(k, j));
