...
public class VehicleDamage : MonoBehaviour
{
	...
	{
		bool flag = IsGiantessDamage(col);
		if (!((hitTime == 0f && col.relativeVelocity.sqrMagnitude * damageFactor > 1f && strength < 1f) || flag))
		{
			return;
		}
		...
		for (int i = 0; i < contacts.Length; i++)
		{
			...
			{
				num++;
				if ((bool)vp.crashSnd && vp.crashClips.Length != 0 && !flag2)
				{
				...
				DamageApplication(colPoint.point, col.relativeVelocity, maxCollisionMagnitude, colPoint.normal, colPoint, true, flag);
			}
	...
	{
		DamageApplication(colPoint.point, colVel, float.PositiveInfinity, colPoint.normal, colPoint, true, isGiantess);
		FinalizeDamage(isGiantess);
	}

	public void ApplyDamage(ContactPoint colPoint, Vector3 colVel, float damageForceLimit, bool isGiantess)
	{
		DamageApplication(colPoint.point, colVel, damageForceLimit, colPoint.normal, colPoint, true, isGiantess);
		FinalizeDamage(isGiantess);
	}

	public void ApplyDamage(Vector3 damagePoint, Vector3 damageForce, bool isGiantess)
	{
		DamageApplication(damagePoint, damageForce, float.PositiveInfinity, damageForce.normalized, nullContact, false, isGiantess);
		FinalizeDamage(isGiantess);
	}

	public void ApplyDamage(Vector3 damagePoint, Vector3 damageForce, float damageForceLimit, bool isGiantess)
	{
		DamageApplication(damagePoint, damageForce, damageForceLimit, damageForce.normalized, nullContact, false, isGiantess);
		FinalizeDamage(isGiantess);
	}
	...
	private void DamageApplication(Vector3 damagePoint, Vector3 damageForce, float damageForceLimit, Vector3 surfaceNormal, ContactPoint colPoint, bool useContactPoint, bool isGiantess)
	{
		...
		for (int j = 0; j < deformMeshes.Length; j++)
		{
			...
			if (!((vector2.sqrMagnitude > 0f && strength < 1f) || isGiantess))
			{
				continue;
			}
			for (int k = 0; k < meshVertices[j].verts.Length; k++)
			{
				...
				if (num6 > 0.001f || isGiantess)
				{
					...
					{
						Vector3 vector4 = (seamlessDeform ? Vector3.zero : Vector3.Project(normalized, meshVertices[j].verts[k]));
						meshVertices[j].verts[k] += (vector3 - vector4 * (usePerlinNoise ? (1f + Mathf.PerlinNoise(meshVertices[j].verts[k].x * 100f, meshVertices[j].verts[k].y * 100f)) : 1f)) * num4 * Mathf.Min(num2 * 0.01f, num6) * num3;
					}
					else
					{
						Vector3 onNormal = transform.InverseTransformPoint(meshFilter.transform.TransformPoint(meshVertices[j].verts[k]));
						meshVertices[j].verts[k] += (vector3 - Vector3.Project(normalized, onNormal) * (usePerlinNoise ? (1f + Mathf.PerlinNoise(onNormal.x * 100f, onNormal.y * 100f)) : 1f)) * num4 * Mathf.Min(num2 * 0.01f, num6) * num3;
					}
				}
			}
		}
		...
		for (int n = 0; n < displaceParts.Length; n++)
		{
			...
			if ((bool)component5 && ((!component5.wheel.grounded && ignoreGroundedWheels) || !ignoreGroundedWheels || isGiantess))
			{
			...
			if ((bool)component6 && ((!component6.grounded && ignoreGroundedWheels) || !ignoreGroundedWheels || isGiantess) && num2 * num4 * num6 * 10f * num3 > component6.detachForce)
			{
				component6.Detach();
			}
		}
