...
public class Suspension : MonoBehaviour
{
	...
	private void Start()
	{
		...
		if (Application.isPlaying)
		{
			...
			if (generateHardCollider)
			{
				...
				compressCol.height = (wheel.popped ? wheel.rimRadius : Mathf.Lerp(wheel.rimRadius, wheel.tireRadius, wheel.tirePressure)) * 2f;
	...
	private void FixedUpdate()
	{
		...
		if (wheel.connected)
		{
			compression = Mathf.Min(targetCompression, (suspensionDistance > 0f) ? Mathf.Clamp01(wheel.contactPoint.distance / suspensionDistance) : 0f);
	...
	private void Update()
	{
		...
		steerDegrees = Mathf.Abs(steerAngle) * ((steerAngle > 0f) ? steerRangeMax : steerRangeMin);
	}
	...
	private void ApplySuspensionForce()
	{
		...
		{
			Vector3 vector = (leaningForce ? Vector3.Lerp(upDir, vp.norm.forward, Mathf.Abs(Mathf.Pow(Vector3.Dot(vp.norm.forward, vp.upDir), 5f))) : vp.norm.forward) * springForce * (Mathf.Pow(springForceCurve.Evaluate(1f - compression), Mathf.Max(1f, springExponent)) - (1f - targetCompression) - springDampening * Mathf.Clamp(z, -1f, 1f));
			rb.AddForceAtPosition(vector, applyForceAtGroundContact ? wheel.contactPoint.point : wheel.tr.position, ForceMode.Acceleration);
		...
		{
			rb.AddForceAtPosition(-vp.norm.TransformDirection(0f, 0f, Mathf.Clamp(z, (0f - hardContactSensitivity) * TimeMaster.fixedTimeFactor, 0f) + penetration) * hardContactForce * Mathf.Clamp01(TimeMaster.fixedTimeFactor), applyForceAtGroundContact ? wheel.contactPoint.point : wheel.tr.position, ForceMode.Acceleration);
		}
	}
	...
	private void GetCamber()
	{
		if (solidAxleCamber && (bool)oppositeWheel && wheel.connected)
		{
			if ((bool)oppositeWheel.wheel.rim && (bool)wheel.rim)
			{
				...
				camberAngle = Mathf.Atan2(vector.z, vector.y) * 57.29578f + 90f + camberOffset;
			}
		}
		else
		{
