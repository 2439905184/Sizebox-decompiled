...
public class StuntDetect : MonoBehaviour
{
	...
	{
		endDriftTime = ((vp.groundedWheels <= 0) ? 0f : ((Mathf.Abs(vp.localVelocity.x) > 5f) ? StuntManager.driftConnectDelayStatic : Mathf.Max(0f, endDriftTime - Time.timeScale * TimeMaster.inverseFixedTimeFactor)));
	...
	private void DetectFlips()
	{
		if (vp.groundedWheels == 0)
		{
			...
			flipString = "";
			{
				...
				{
					text = ((doneStunt2.progress * 57.29578f >= doneStunt2.angleThreshold * 2f) ? (" x" + Mathf.FloorToInt(doneStunt2.progress * 57.29578f / doneStunt2.angleThreshold)) : "");
					flipString = (string.IsNullOrEmpty(flipString) ? (doneStunt2.name + text) : (flipString + " + " + doneStunt2.name + text));
				}
				return;
			}
		}
