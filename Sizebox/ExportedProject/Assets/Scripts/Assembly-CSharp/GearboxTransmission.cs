...
public class GearboxTransmission : Transmission
{
	...
	private void FixedUpdate()
	{
		...
		{
			newDrive.rpm = ((automatic && skidSteerDrive) ? (Mathf.Abs(targetDrive.rpm) * Mathf.Sign(vp.accelInput - (vp.brakeIsReverse ? (vp.brakeInput * (1f - vp.burnout)) : 0f))) : targetDrive.rpm) / curGearRatio;
		...
		if (automatic && shiftTime == 0f && vp.groundedWheels > 0)
		{
			if (!skidSteerDrive && vp.burnout == 0f)
			{
				...
				{
					if (currentGear < gears.Length - 1 && (upperGear.minRPM + upshiftDifference * ((curGearRatio < 0f) ? Mathf.Min(1f, shiftThreshold) : shiftThreshold) - num <= 0f || (curGearRatio <= 0f && upperGear.ratio > 0f && (!vp.reversing || (vp.accelInput > 0f && vp.localVelocity.z > curGearRatio * 10f)))) && (!(vp.brakeInput > 0f) || !vp.brakeIsReverse || !(upperGear.ratio >= 0f)) && (!(vp.localVelocity.z < 0f) || vp.accelInput != 0f))
					{
						Shift(1);
					}
