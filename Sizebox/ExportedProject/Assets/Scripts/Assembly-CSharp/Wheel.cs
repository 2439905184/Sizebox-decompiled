...
public class Wheel : MonoBehaviour
{
		...
		if (tr.childCount > 0)
		{
			...
			if (rim.childCount > 0)
			{
				...
				if (canDetach)
				{
					...
					detachedTire.AddComponent<MeshRenderer>().sharedMaterial = (tireMat ? tireMat : tire.GetComponent<MeshRenderer>().sharedMaterial);
				}
			}
	...
	private void FixedUpdate()
	{
		...
		actualEbrake = (suspensionParent.ebrakeEnabled ? suspensionParent.ebrakeForce : 0f);
		actualTargetRPM = targetDrive.rpm * (float)((!suspensionParent.driveInverted) ? 1 : (-1));
		actualTorque = (suspensionParent.driveEnabled ? Mathf.Lerp(targetDrive.torque, Mathf.Abs(vp.accelInput), vp.burnout) : 0f);
		if (getContact)
		{
			GetWheelContact();
		}
		...
		{
			rb.AddForceAtPosition(suspensionParent.forwardDir * (0f - suspensionParent.flippedSideFactor) * (vp.steerInput * vp.burnoutSpin * currentRPM * Mathf.Min(0.1f, targetDrive.torque) * 0.001f) * vp.burnout * (popped ? 0.5f : 1f) * contactPoint.surfaceFriction, suspensionParent.tr.position, ForceMode.Acceleration);
		}
		setPopped = popped;
	...
	private void Update()
	{
		...
		{
			Vector3 vector = (grounded ? (contactPoint.normal * Mathf.Max((0f - suspensionParent.penetration) * (1f - suspensionParent.compression) * 10f, 1f - tirePressure) * deformAmount) : Vector3.zero);
		...
		{
			float num = ((connected && GroundSurfaceMaster.surfaceTypesStatic[contactPoint.surfaceType].leaveSparks) ? Mathf.Abs(F.MaxAbs(forwardSlip, sidewaysSlip)) : 0f);
			glowAmount = (popped ? Mathf.Lerp(glowAmount, num, ((num > glowAmount) ? 2f : 0.2f) * Time.deltaTime) : 0f);
	...
	private void ApplyFriction()
	{
		if (grounded)
		{
			float f = ((slipDependence == SlipDependenceMode.dependent || slipDependence == SlipDependenceMode.forward) ? (forwardSlip - sidewaysSlip) : forwardSlip);
			float f2 = ((slipDependence == SlipDependenceMode.dependent || slipDependence == SlipDependenceMode.sideways) ? (sidewaysSlip - forwardSlip) : sidewaysSlip);
			...
			frictionForce = Vector3.Lerp(frictionForce, tr.TransformDirection(forwardFrictionCurve.Evaluate(Mathf.Abs(f)) * (float)(-Math.Sign(forwardSlip)) * (popped ? forwardRimFriction : forwardFriction) * num * (0f - suspensionParent.flippedSideFactor), 0f, sidewaysFrictionCurve.Evaluate(Mathf.Abs(f2)) * (float)(-Math.Sign(sidewaysSlip)) * (popped ? sidewaysRimFriction : sidewaysFriction) * num2 * normalFrictionCurve.Evaluate(Mathf.Clamp01(Vector3.Dot(contactPoint.normal, GlobalControl.worldUpDir))) * ((vp.burnout > 0f && Mathf.Abs(targetDrive.rpm) != 0f && actualEbrake * vp.ebrakeInput == 0f && grounded) ? ((1f - vp.burnout) * (1f - Mathf.Abs(vp.accelInput))) : 1f)) * (0.5f + (1f - suspensionParent.compression) * 0.5f * Mathf.Clamp01(Mathf.Abs(suspensionParent.tr.InverseTransformDirection(localVel).z) * 10f)) * contactPoint.surfaceFriction, 1f - frictionSmoothness);
	...
	{
		float num = 0f;
		float num2 = (suspensionParent.skidSteerBrake ? vp.localAngularVel.y : vp.localVelocity.z);
		...
		if (!suspensionParent.jammed && connected)
		{
			...
			currentRPM = Mathf.Lerp(rawRPM, Mathf.Lerp(Mathf.Lerp(rawRPM, actualTargetRPM, flag ? EvaluateTorque(actualTorque) : actualTorque), 0f, Mathf.Max(num, actualEbrake * vp.ebrakeInput)), flag ? EvaluateTorque(actualTorque + num + actualEbrake * vp.ebrakeInput) : (actualTorque + num + actualEbrake * vp.ebrakeInput));
	...
	private void PositionWheel()
	{
		...
		{
			rim.position = suspensionParent.maxCompressPoint + suspensionParent.springDirection * suspensionParent.suspensionDistance * (Application.isPlaying ? travelDist : suspensionParent.targetCompression) + suspensionParent.upDir * Mathf.Pow(Mathf.Max(Mathf.Abs(Mathf.Sin(suspensionParent.sideAngle * ((float)Math.PI / 180f))), Mathf.Abs(Mathf.Sin(suspensionParent.casterAngle * ((float)Math.PI / 180f)))), 2f) * actualRadius + suspensionParent.pivotOffset * suspensionParent.tr.TransformDirection(Mathf.Sin(tr.localEulerAngles.y * ((float)Math.PI / 180f)), 0f, Mathf.Cos(tr.localEulerAngles.y * ((float)Math.PI / 180f))) - suspensionParent.pivotOffset * (Application.isPlaying ? suspensionParent.forwardDir : suspensionParent.tr.forward);
		}
		if (Application.isPlaying && generateHardCollider)
		{
			bool connected2 = connected;
		}
	}
	...
	public void Detach()
	{
		if (connected && canDetach)
		{
			...
			if ((bool)tire)
			{
				...
				detachedCol.sharedMesh = ((!(airLeakTime >= 0f) && !popped) ? (tireMeshLoose ? tireMeshLoose : detachTireFilter.sharedMesh) : (rimMeshLoose ? rimMeshLoose : detachFilter.sharedMesh));
			}
			else
			{
	...
	private void OnDrawGizmosSelected()
	{
		...
		{
			Gizmos.color = new Color(1f, 1f, 0f, popped ? 0.5f : 1f);
		...
		Gizmos.color = ((tirePressure == 0f || popped) ? Color.green : Color.cyan);
		GizmosExtra.DrawWireCylinder(rim.position, rim.forward, rimRadius, rimWidth * 2f);
		Gizmos.color = new Color(1f, 1f, 1f, (tirePressure < 1f) ? 0.5f : 1f);
		GizmosExtra.DrawWireCylinder(rim.position, rim.forward, tireRadius, tireWidth * 2f);
		Gizmos.color = ((tirePressure == 0f || popped) ? Color.green : Color.cyan);
		GizmosExtra.DrawWireCylinder(rim.position, rim.forward, rimRadius, rimWidth * 2f);
	}
