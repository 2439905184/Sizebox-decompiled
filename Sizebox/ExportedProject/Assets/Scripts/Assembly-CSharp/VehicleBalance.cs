...
public class VehicleBalance : MonoBehaviour
{
	...
	{
		actualPitchInput = ((vp.wheels.Length == 1) ? 0f : Mathf.Clamp(vp.pitchInput, -1f, (!(vp.velMag > endoSpeedThreshold)) ? 1 : 0));
	...
	private void ApplyLean()
	{
		...
		{
			Vector3 direction = vp.norm.InverseTransformDirection((Vector3.Dot(vp.wheelNormalAverage, GlobalControl.worldUpDir) <= 0f) ? vp.wheelNormalAverage : Vector3.Lerp(GlobalControl.worldUpDir, vp.wheelNormalAverage, Mathf.Abs(Vector3.Dot(vp.norm.up, GlobalControl.worldUpDir)) * 2f));
		...
		rb.AddTorque(vp.norm.right * (0f - (Vector3.Dot(vp.forwardDir, targetLeanActual) * 20f - vp.localAngularVel.x)) * 100f * ((vp.wheels.Length == 1) ? 1f : leanPitchCurve.Evaluate(Mathf.Abs(actualPitchInput))), ForceMode.Acceleration);
		rb.AddTorque(vp.norm.forward * ((vp.groundedWheels == 1) ? (vp.steerInput * leanFactor.y - vp.norm.InverseTransformDirection(rb.angularVelocity).z) : 0f) * 100f * leanYawCurve.Evaluate(Mathf.Abs(vp.steerInput)), ForceMode.Acceleration);
