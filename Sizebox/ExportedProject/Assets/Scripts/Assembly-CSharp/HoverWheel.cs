...
public class HoverWheel : MonoBehaviour
{
	...
	{
		float num = ((vp.localVelocity.z > 0f) ? vp.brakeInput : Mathf.Clamp01(vp.accelInput)) * brakeForce + vp.ebrakeInput * ebrakeForce;
	...
	private void TiltWheel()
	{
		...
		float num2 = ((vp.localVelocity.z > 0f) ? vp.brakeInput : Mathf.Clamp01(vp.accelInput)) * brakeForce + vp.ebrakeInput * ebrakeForce;
		...
		visualWheel.localRotation = Quaternion.Lerp(visualWheel.localRotation, Quaternion.LookRotation(new Vector3((0f - num3) * visualTiltAmount, -1f + Mathf.Abs(F.MaxAbs(num, num3)) * visualTiltAmount, (0f - num) * visualTiltAmount).normalized, Vector3.forward), visualTiltRate * Time.deltaTime);
	}
