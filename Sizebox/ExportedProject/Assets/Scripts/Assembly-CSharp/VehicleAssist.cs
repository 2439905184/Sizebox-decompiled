...
public class VehicleAssist : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		...
		{
			groundedFactor = ((!basedOnWheelsGrounded) ? 1 : (vp.groundedWheels / (vp.hover ? vp.hoverWheels.Length : vp.wheels.Length)));
			angDragTime = 20f;
		...
		else if (angularDragOnJump)
		{
			...
			rb.angularDrag = ((angDragTime > 0f && (double)vp.upDot > 0.5) ? 10f : initialAngularDrag);
		}
		if (downforce > 0f)
		{
			ApplyDownforce();
	...
	private void ApplySpinAssist()
	{
		...
		if (autoSteerDrift)
		{
			...
			targetDriftAngle = (((float)num2 != Mathf.Sign(vp.localVelocity.x)) ? vp.steerInput : ((float)num2)) * (0f - maxDriftAngle);
		...
		{
			num = vp.steerInput * driftSpinSpeed * ((!(vp.localVelocity.z < 0f)) ? 1f : (vp.accelAxisIsBrake ? Mathf.Sign(vp.accelInput) : Mathf.Sign(F.MaxAbs(vp.accelInput, 0f - vp.brakeInput))));
		}
	...
	private void ApplyDownforce()
	{
		...
		{
			rb.AddRelativeForce(new Vector3(0f, downforceCurve.Evaluate(Mathf.Abs(vp.localVelocity.z)) * (0f - downforce) * (applyDownforceInAir ? 1f : groundedFactor) * (invertDownforceInReverse ? Mathf.Sign(vp.localVelocity.z) : 1f), 0f), ForceMode.Acceleration);
			if (invertDownforceInReverse && vp.localVelocity.z < 0f)
			{
				rb.AddRelativeTorque(new Vector3(downforceCurve.Evaluate(Mathf.Abs(vp.localVelocity.z)) * downforce * (applyDownforceInAir ? 1f : groundedFactor), 0f, 0f), ForceMode.Acceleration);
			}
		}
	}
	...
	{
		float f = (vp.accelAxisIsBrake ? vp.accelInput : (vp.accelInput - vp.brakeInput)) * Mathf.Abs(vp.localVelocity.x) * driftPush * groundedFactor * (1f - Mathf.Abs(Vector3.Dot(vp.forwardDir, rb.velocity.normalized)));
