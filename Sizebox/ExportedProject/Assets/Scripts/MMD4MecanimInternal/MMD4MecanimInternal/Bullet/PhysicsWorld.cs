...
namespace MMD4MecanimInternal.Bullet
{
	public class PhysicsWorld
	{
		...
		{
			public PhysicsWorld _003C_003E4__this;

			public float deltaTime;

			public void _003C_Invoke_003Eb__0()
			{
				_003C_003E4__this._InternalUpdate(deltaTime);
			}
		}
		...
		public bool Create(WorldProperty worldProperty)
		{
			...
			if (_worldProperty.axisSweepDistance > 0f)
			{
				...
				_broadphase = new AxisSweep3Internal(ref worldAabbMin, ref worldAabbMax, 65534, ushort.MaxValue, 16384, null, false);
			}
			else
			{
		...
		public void Update(float deltaTime, WorldUpdateProperty worldUpdateProperty)
		{
			...
			if (_gravityNoise != 0f)
			{
				...
				if (_gravityNoiseTime >= 0.2f)
				{
					...
					_gravityNoiseRate += (UnityEngine.Random.Range(0f, 1f) - 0.5f) * 0.5f;
		...
		private void _Invoke(float deltaTime)
		{
			...
			_003C_003Ec__DisplayClass.deltaTime = deltaTime;
			_003C_003Ec__DisplayClass._003C_003E4__this = this;
			_isRunningThread = true;
			if (_cachedThreadQueue != null && Global.bridge != null)
			{
				IBridge bridge = Global.bridge;
				object cachedThreadQueue = _cachedThreadQueue;
				if (action == null)
				{
					action = _003C_003Ec__DisplayClass._003C_Invoke_003Eb__0;
				}
				_threadQueueHandle = bridge.InvokeCachedThreadQueue(cachedThreadQueue, action);
			}
		}
