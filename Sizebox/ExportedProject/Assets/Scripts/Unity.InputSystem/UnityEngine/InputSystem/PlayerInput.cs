	...
	public class PlayerInput : MonoBehaviour
	{
		...
		public string currentControlScheme
		{
			get
			{
				...
				InputControlScheme? controlScheme = m_InputUser.controlScheme;
		...
		public InputActionMap currentActionMap
		{
			...
			set
			{
				...
				InputActionMap inputActionMap2 = m_CurrentActionMap;
		...
		public static ReadOnlyArray<PlayerInput> all
		{
			get
			{
				return new ReadOnlyArray<PlayerInput>(s_AllActivePlayers, 0, s_AllActivePlayersCount);
			}
		}
		...
		public void SwitchCurrentActionMap(string mapNameOrId)
		{
			...
			if (inputActionMap == null)
			{
				Debug.LogError(string.Format("Cannot find action map '{0}' in actions '{1}'", mapNameOrId, m_Actions), this);
			}
			else
			{
			...
			case PlayerNotifications.InvokeUnityEvents:
			{
				...
				foreach (ActionEvent actionEvent in array)
				{
					...
					if (!string.IsNullOrEmpty(actionId))
					{
						...
						{
							Debug.LogError(string.Format("Cannot find action '{0}' with ID '{1}' in '{2}", actionEvent.actionName, actionEvent.actionId, m_Actions), this);
						}
						else
						{
							Debug.LogError(string.Format("Cannot find action with ID '{0}' in '{1}", actionEvent.actionId, m_Actions), this);
						}
					}
				}
				break;
		...
		private void AssignUserAndDevices()
		{
			...
			if (m_Actions.controlSchemes.Count > 0)
			{
				if (!string.IsNullOrEmpty(s_InitControlScheme))
				{
					...
					if (!inputControlScheme.HasValue)
					{
						Debug.LogError(string.Format("No control scheme '{0}' in '{1}'", s_InitControlScheme, m_Actions), this);
					}
					else
					{
				...
				else if (!string.IsNullOrEmpty(m_DefaultControlScheme))
				{
					...
					{
						Debug.LogError(string.Format("Cannot find default control scheme '{0}' in '{1}'", m_DefaultControlScheme, m_Actions), this);
					}
					else
					{
				...
				{
					InputControlScheme? inputControlScheme3 = InputControlScheme.FindControlSchemeForDevices(new ReadOnlyArray<InputDevice>(s_InitPairWithDevices, 0, s_InitPairWithDevicesCount), m_Actions.controlSchemes, null, true);
		...
		private void HandleDeviceLost()
		{
			...
			case PlayerNotifications.InvokeUnityEvents:
			{
				DeviceLostEvent obj = m_DeviceLostEvent;
				if (obj != null)
				{
					obj.Invoke(this);
				}
		...
		private void HandleDeviceRegained()
		{
			...
			case PlayerNotifications.InvokeUnityEvents:
			{
				DeviceRegainedEvent obj = m_DeviceRegainedEvent;
				if (obj != null)
				{
					obj.Invoke(this);
				}
		...
		private void HandleControlsChanged()
		{
			...
			case PlayerNotifications.InvokeUnityEvents:
			{
				ControlsChangedEvent obj = m_ControlsChangedEvent;
				if (obj != null)
				{
					obj.Invoke(this);
				}
		...
		private void OnUnpairedDeviceUsed(InputControl control, InputEventPtr eventPtr)
		{
			...
			using (InputActionRebindingExtensions.DeferBindingResolution())
			{
				using (InputControlList<InputDevice> inputControlList = InputUser.GetUnpairedInputDevices())
				{
					...
					if (!InputControlScheme.FindControlSchemeForDevices(inputControlList, playerInput.m_Actions.controlSchemes, out controlScheme, out matchResult, device))
					{
						return;
					}
					try
