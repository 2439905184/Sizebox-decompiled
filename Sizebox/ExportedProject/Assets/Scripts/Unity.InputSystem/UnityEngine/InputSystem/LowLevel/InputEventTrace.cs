	...
	public sealed class InputEventTrace : IDisposable, IEnumerable<InputEventPtr>, IEnumerable
	{
		...
		public class ReplayController : IDisposable
		{
			...
			private sealed class _003C_003Ec
			{
				...
				public static Comparison<InputEventPtr> _003C_003E9__38_0;

				internal int _003CPlayAllEventsAccordingToTimestamps_003Eb__38_0(InputEventPtr a, InputEventPtr b)
				{
			...
			private sealed class _003C_003Ec__DisplayClass43_0
			{
				...
				internal bool _003CApplyDeviceMapping_003Eb__0(DeviceInfo x)
				{
					return x.deviceId == originalDeviceId;
				}
			}
			...
			public ReplayController PlayAllEvents()
			{
				...
				{
					InputEventPtr eventPtr;
					while (MoveNext(true, out eventPtr))
					{
						QueueEvent(eventPtr);
					}
				}
			...
			public ReplayController PlayAllEventsAccordingToTimestamps()
			{
				...
				list.Sort(_003C_003Ec._003C_003E9__38_0 ?? (_003C_003Ec._003C_003E9__38_0 = _003C_003Ec._003C_003E9._003CPlayAllEventsAccordingToTimestamps_003Eb__38_0));
			...
			private void OnBeginFrame()
			{
				...
				InputEventPtr eventPtr;
				if (!MoveNext(false, out eventPtr))
				{
				...
				{
					QueueEvent(eventPtr);
					InputEventPtr eventPtr2;
					if (!MoveNext(false, out eventPtr2))
					{
						if (m_AllEventsByTime == null || m_AllEventsByTimeIndex >= m_AllEventsByTime.Count)
						{
							Finished();
			...
			private void Finished()
			{
				finished = true;
				InputSystem.onBeforeUpdate -= OnBeginFrame;
				Action onFinished = m_OnFinished;
				if (onFinished != null)
				{
					onFinished();
				}
			...
			private void QueueEvent(InputEventPtr eventPtr)
			{
				...
				Action<InputEventPtr> onEvent = m_OnEvent;
				if (onEvent != null)
				{
					onEvent(eventPtr);
				}
			...
			private int ApplyDeviceMapping(int originalDeviceId)
			{
				...
				_003C_003Ec__DisplayClass43_.originalDeviceId = originalDeviceId;
				for (int i = 0; i < m_DeviceIDMappings.length; i++)
				{
					KeyValuePair<int, int> keyValuePair = m_DeviceIDMappings[i];
					if (keyValuePair.Key == _003C_003Ec__DisplayClass43_.originalDeviceId)
					{
				...
				if (m_CreateNewDevices)
				{
					try
					{
						int num = m_EventTrace.deviceInfos.IndexOf(_003C_003Ec__DisplayClass43_._003CApplyDeviceMapping_003Eb__0);
						if (num != -1)
						{
							...
							if (!InputControlLayout.s_Layouts.HasLayout(internedString))
							{
								...
								{
									return _003C_003Ec__DisplayClass43_.originalDeviceId;
								}
								InputSystem.RegisterLayout(deviceInfo.m_FullLayoutJson);
							}
							InputDevice inputDevice = InputSystem.AddDevice(internedString);
							WithDeviceMappedFromTo(_003C_003Ec__DisplayClass43_.originalDeviceId, inputDevice.deviceId);
		...
		public unsafe void ReadFrom(Stream stream)
		{
			...
			{
				throw new IOException(string.Format("Stream does not appear to be an InputEventTrace (no '{0}' code)", kFileFormat));
			}
			if (binaryReader.ReadInt32() > kFileVersion)
			{
				throw new IOException(string.Format("Stream is an InputEventTrace but a newer version (expected version {0} or below)", kFileVersion));
			}
